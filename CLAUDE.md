...
# ユニットテスト（Vitest、src/lib/ の純粋関数）
pnpm test

# E2Eテスト（Playwright、ビルド込み。ビルド済みなら pnpm test:e2e:only）
pnpm test:e2e

# 本物の github.com に対するE2E（GitHubのUI変更検知。ビルド済みで実行）
pnpm test:e2e:real
...
- **ユニットテスト（Vitest）**: URL検証・サニタイズ・エスケープ・ページ判定などの純粋関数は`src/lib/`（`imageUrls.ts`, `githubPage.ts`）に切り出し、`src/lib/*.test.ts`でテストする。DOMに触るロジックを足すときも、純粋な部分は`src/lib/`に置くこと
...
- **本物のGitHubに対するチェック（`e2e/real/`、`pnpm test:e2e:real`）**: 公開PR（#112）の差分ページに拡張機能を実際に注入して、GitHub側のUI変更による破壊を検知する。`playwright.real.config.ts`で分離し、通常の`pnpm test:e2e`からは除外。`ci.yml`の`real-github`ジョブがPRごとに実行する（手動実行も可）
  - 未ログインだとGitHubは旧UI（`/files`、`.pr-review-tools`）しか返さないので、新UI（`/changes`）の検証にはログイン状態が必要。`pnpm auth:github`でブラウザからログインして`.github-auth.json`（gitignore済み）を作り、その中身をリポジトリシークレット`GH_UI_CHECK_STORAGE_STATE`に登録すると新UIのテストも走る。未設定ならスキップされる
